...
1. Using the same approach as we did earlier in this lesson to join `surveys` and `tail`, start with the `surveys` dataframe and use a join to merge in data from two additional tables: a `species` table found at "https://ucd-rdavis.github.io/R-DAVIS/data/species.csv" and a `plots` table found at "https://ucd-rdavis.github.io/R-DAVIS/data/plots.csv". 
Hint: you will have to perform these two joins one at a time. 
Call the resulting data frame `surveys_spp_and_plots`.
2. Use `pivot_wider` on the `surveys_spp_and_plots` data frame with `year` as columns, `plot_id` as rows, and the number of genera per plot as the values. You will need to summarize before
  reshaping, and use the function `n_distinct()` to get the number of unique
  genera within a particular chunk of data.  
3. The `surveys_spp_and_plots` data set will have two measurement columns: `hindfoot_length` and `weight`.  This makes it
...
```{r, purl=FALSE}
## Answer 1
species <- read_csv("https://ucd-rdavis.github.io/R-DAVIS/data/species.csv")
plots <- read_csv("https://ucd-rdavis.github.io/R-DAVIS/data/plots.csv")
surveys_and_spp <- left_join(surveys, species)
surveys_spp_and_plots <- left_join(surveys_and_spp, plots)

## Answer 2
q2 <- surveys_spp_and_plots %>%
