...
str(tail)
```
Looks like we have a data frame with 34,786 rows and two columns: `record_id` and `tail_length`.
Recall the structure of our `surveys` data frame:
```{r}
str(surveys)
```
Luckily, both our `surveys` and `tail` data frames have the column `record_id`. We can use this shared column to `join` the two data frames together, so that we can add the `tail_length` variable to our `surveys` data frame. 
...
You might have noticed that our `tail` data frame had fewer rows than our `surveys` data frame. Some of our observations in `surveys` must be missing tail length measurements. Since we are using a `left_join`, any observations in `surveys` that do not have a match in `tail` will be given an NA value for `tail_length`. 
...
surveys_joined
```
We now have a new data frame called `surveys_joined` which has all of the information from our original `surveys` data frame, plus the new `tail_length` column that we added from the `tail` data frame!
