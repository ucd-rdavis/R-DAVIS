...
- [Dates with Lubridate](https://video.ucdavis.edu/media/Week8%3A+Dates+and+Times+with+Lubridate/1_dvn2txzq/390286133)
- [Function-writing](https://video.ucdavis.edu/media/Week8_Part2_WritingFunctions/1_fvrbfw9h/390286133)
- [Video Discussion](https://canvas.ucdavis.edu/courses/1098028/assignments/1510685)

## In Class Coding

[Submit Week 8 In Class Coding](https://canvas.ucdavis.edu/courses/1098028/assignments/1509413)
...
In this lesson, we are going to introduce how base R deals with dates (`POSIXct` or `POSIXlt`), but we are going to spend the majority of our lesson on the package `lubridate`. `lubridate` is a great package that makes it much easier to work with dates and times in R. 
...
Importantly, there are **3** basic time classes in R:
 ...
 - **`POSIXlt`** ("**lt**" == local time, enables easy extraction of specific components of a time, however, remember that POSIXlt objects are lists)
...
nfy1 <- read_csv("https://ucd-rdavis.github.io/R-DAVIS/data/2015_NFY_solinst.csv", skip = 12)
...
nfy2 <- read_csv("https://ucd-rdavis.github.io/R-DAVIS/data/2015_NFY_solinst.csv", col_types = "ccidd", skip=12)
...
functions.R in the `scripts` directory of your project.
...
You can load all the functions in your `scripts/functions.R` script without even opening the file, via the `source` function. This allows you to keep your functions separate from the analyses which use them. 
